import logoAsset from "@/assets/multitech-logo.jpg.asset.json";
import engineeringTeamAsset from "@/assets/engineering-team.jpg.asset.json";
import projectPlanningAsset from "@/assets/project-planning.jpg.asset.json";
import civilEngineerAsset from "@/assets/civil-engineer.jpg.asset.json";
import solarTechnicianAsset from "@/assets/solar-technician.jpg.asset.json";
import fieldEngineerAsset from "@/assets/field-engineer.jpg.asset.json";
import mechanicalWelderAsset from "@/assets/mechanical-welder.jpg.asset.json";
import electricalTechnicianAsset from "@/assets/electrical-technician.jpg.asset.json";
import electricalMaintenanceAsset from "@/assets/electrical-maintenance.jpg.asset.json";
import telecomInfrastructureAsset from "@/assets/telecom-infrastructure.jpg.asset.json";
import engineeringCollaborationAsset from "@/assets/engineering-collaboration.jpg.asset.json";
import solarInstallersAsset from "@/assets/solar-installers.jpg.asset.json";
import serviceCivilAsset from "@/assets/service-civil.png.asset.json";
import serviceBuildingAsset from "@/assets/service-building.png.asset.json";
import serviceElectricalAsset from "@/assets/service-electrical.png.asset.json";
import serviceSolarAsset from "@/assets/service-solar.png.asset.json";
import serviceHvacAsset from "@/assets/service-hvac.png.asset.json";
import serviceMechanicalAsset from "@/assets/service-mechanical.png.asset.json";
import serviceIctSecurityAsset from "@/assets/service-ict-security.png.asset.json";
import industrialElectricalSystemAsset from "@/assets/industrial-electrical-system.png.asset.json";
import infrastructureDevelopmentAsset from "@/assets/infrastructure-development.png.asset.json";
import solarPowerInstallationAsset from "@/assets/solar-power-installation.png.asset.json";
import commercialOfficeComplexAsset from "@/assets/commercial-office-complex.png.asset.json";

export const siteAssets = {
  logo: logoAsset.url,
  engineeringTeam: engineeringTeamAsset.url,
  projectPlanning: projectPlanningAsset.url,
  civilEngineer: civilEngineerAsset.url,
  solarTechnician: solarTechnicianAsset.url,
  fieldEngineer: fieldEngineerAsset.url,
  mechanicalWelder: mechanicalWelderAsset.url,
  electricalTechnician: electricalTechnicianAsset.url,
  electricalMaintenance: electricalMaintenanceAsset.url,
  telecomInfrastructure: telecomInfrastructureAsset.url,
  engineeringCollaboration: engineeringCollaborationAsset.url,
  solarInstallers: solarInstallersAsset.url,
  serviceCivil: serviceCivilAsset.url,
  serviceBuilding: serviceBuildingAsset.url,
  serviceElectrical: serviceElectricalAsset.url,
  serviceSolar: serviceSolarAsset.url,
  serviceHvac: serviceHvacAsset.url,
  serviceMechanical: serviceMechanicalAsset.url,
  serviceIctSecurity: serviceIctSecurityAsset.url,
};